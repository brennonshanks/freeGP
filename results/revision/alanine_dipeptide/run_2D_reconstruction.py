...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=2 * np.pi,
        help="Periodicity (radians) of each collective variable, for minimal-image wrapping of "
        "samples around each window's own center. Both dihedral CVs here are periodic on "
        "(-pi, pi]; pass 0 to disable wrapping.",
    )
    parser.add_argument("--num-test-points", type=int, default=25, help="Test-grid points per dimension.")
    parser.add_argument("--fixed-ell", type=float, default=np.pi/3, help="Fixed length scale for the GP.")
    parser.add_argument("--fixed-w", type=float, default=20.0, help="Fixed signal variance for the GP.")
    ...
    bundle = prepare_gprhd_inputs_nd(
        ...
        file_suffix="_xyplane.xvg",
        period=args.period if args.period > 0 else None,
    )
    ...
    mean_panels = [("WHAM", ref, None), ("Fixed GP", fixed_mean, fixed_std), ("MAP GP", map_mean, map_std)]
    if hmc_mean is not None:
