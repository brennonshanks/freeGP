...
def ind2sub_RMO(sz,ind):
	...
	# Signed indices are required below when neighboring bins are addressed
	# with an offset of -1. NumPy 2 rejects that operation for uint64 arrays.
	sub = np.zeros(sz.shape[0], dtype=np.int64)
...
for step in range(0,steps_MH):
	...
	if ( (step+1) % saveMod_MH == 0 ):
		# Python 2 used integer division here; preserve that sample index.
