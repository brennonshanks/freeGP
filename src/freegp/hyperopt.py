...
def _stationary_posterior_nd(
    ...
    *,
    ...
    if fixed_noise:
        function_noise = observations.noise_func_cov
        derivative_noise = observations.noise_deriv_cov
    else:
        function_noise = params["sigma_f"].square() * torch.eye(n_obs, dtype=dtype, device=device)
        eye_d = torch.eye(n_dim, dtype=dtype, device=device)
        derivative_noise = params["sigma_d"].square() * eye_d.unsqueeze(0).expand(n_windows, n_dim, n_dim)
    return build_joint_gp_nd(
        ...
        w=params["w"],
        noise_func_cov=function_noise,
        noise_deriv_cov=derivative_noise,
        H_func=observations.H_obs,
        jitter=jitter,
    )


def optimize_stationary_hyperparameters_nd(
    observations: JointObservationsND,
    *,
    objective: str = "lml",
    priors: HyperPriorConfig | None = None,
    ...
    """ND generalization of :func:`optimize_stationary_hyperparameters`.

    Fits a single isotropic length scale ``ell`` shared across all D input
    dimensions (see :func:`freegp.gp.build_joint_gp_nd`); everything else
    mirrors the 1D optimizer.
    """
    if objective not in {"lml", "loo"}:
        raise ValueError("objective must be 'lml' or 'loo'.")
    if steps <= 0 or restarts <= 0:
    ...
    for restart in range(restarts):
        ...
        for _ in range(steps):
            ...
            if not fixed_noise:
                ...
                params["sigma_d"] = values[3]
            try:
                posterior = _stationary_posterior_nd(
                    observations,
                    params,
                    jitter=jitter,
                    fixed_noise=fixed_noise,
        ...
        with torch.no_grad():
            ...
            if not fixed_noise:
                final_params["sigma_f"] = values[2].clone()
                final_params["sigma_d"] = values[3].clone()
            final_posterior = _stationary_posterior_nd(
                observations,
                final_params,
                jitter=jitter,
                fixed_noise=fixed_noise,
